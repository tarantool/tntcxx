...
use the `example application <https://github.com/tarantool/tntcxx/blob/master/examples/Simple.cpp>`_
...
* :ref:`installing and building the Tarantool C++ connector <gs_cxx_prereq_install>`
* :ref:`starting Tarantool and creating a database <gs_cxx_prereq_tnt_run>`
* :ref:`seting up access rights <gs_cxx_prereq_access>`.
...
Currently supported OS for the Tarantool C++ connector usage is Linux.
To install the connector in your OS, you should build it from the source.
...
#. Make sure you have the necessary third-party software. If you miss some of
   the items, install them:

   * `install <https://git-scm.com/book/en/v2/Getting-Started-Installing-Git>`_
      ``git``, a version control system
   * `install <https://linuxize.com/post/how-to-unzip-files-in-linux/#installing-unzip>`_
      the ``unzip`` utility
   * `install <https://gcc.gnu.org/install/>`_ the ``gcc`` compiler `complied
      with the C++17 standard <https://gcc.gnu.org/projects/cxx-status.html#cxx17>`_.
   * `install <https://cmake.org/install/>`_ the ``cmake`` and ``make`` tools.

#. If you don't have Tarantool in you OS, install it in one of the ways:
...
#. Clone the Tarantool C++ connector repository and build the connector from
   the source.
...
.. IMPORTANT::
   ...
   You will need it later to connect to Tarantool from our C++ application.
...
To set up connection to a Tarantool instance from our C++ application, we need
to specify the following in the application code:

* :ref:`to embed the connector into the application <gs_cxx_connect_embed>`
* :ref:`to instantiate a connector client and a connection object <gs_cxx_connect_instantiate>`
* :ref:`to define connection parameters and invoke the method to connect <gs_cxx_connect_connecting>`
* :ref:`to define error handling behavior <gs_cxx_connect_error>`.
...
The connector is embedded in a C++ application by including the main header:
...
The buffer is parametrized by allocator, which means that users are able
to choose which allocator will be used to provide memory for buffer's blocks.
Data is originized into linked list of blocks of fixed size that is specified
...
also need creating connection objects. A connection also takes the buffer and
...
At this step, requests are encoded in the msgpack format and saved in the
...
The ``wait()`` method takes the connection to poll (both IN and OUT),
...
the method returns ``nullptr``. Note that on each future,
...
into the msgpack tuples. To decode client's data, a user has to write one's own
...
.. literalinclude:: ./_includes/Reader.cpp
...
enough to provide a byte-to-byte copy. Since there are
...
It's also important to understand that a tuple itself is wrapped into an array,
...
for our `example application <https://github.com/tarantool/tntcxx/blob/master/examples/Simple.cpp>`_).
